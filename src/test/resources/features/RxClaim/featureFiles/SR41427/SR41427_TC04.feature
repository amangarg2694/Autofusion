...
As a RxClaim User I want to check whether claim gets rejected when fills goes beyond limits 
    
Scenario Outline: To verify if a claim is passed when the period type is 'B' on the GPI/NDC list and the number of fills total starting from the PTD from date is less than or equal to the NDC/GPI number of fills limit 

		Given I am on RxClaim PlanAdministrator Menu 
		When I select Option "4" to navigate to "RxClaim Plan Maintenance"
		And I select Option "2" to navigate to "RxClaim Product List"
		Then I edit the NDC_GPI list with "<NDCList>","<MaxLimit>"
		And I press "F12" Key
