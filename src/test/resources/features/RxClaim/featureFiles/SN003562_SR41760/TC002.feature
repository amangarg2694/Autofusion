...
Scenario Outline: SN003331_SR41449_TC002_Verify Change Plan edit for Member Eligibility when update flag set to Y and Member Eligibility set to D
    ...
    When I enter query
	  ...
	  And I navigate to "<PlanCode>" Plan Details Page
		Then I verify Change Plan edit for Member Eligibility "<MemberEligibility>" when update flag set to Y and Member Eligibility set to D
		# CALL PGM(RCPLN711) PARM('') 
		       
    Examples: 
      | PlanCode  | FromDate | MemberEligibility  | Flag	|	DynamicPA	|
      | SN003562A | 010118	 | Y									|	Y			|	D					|
