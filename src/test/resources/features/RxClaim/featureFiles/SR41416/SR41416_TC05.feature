...
As a RxClaim User I want to check claim is paid and POS rebate are deducted properly 
  ...
  Scenario Outline: To check if the POS rebate is adjusted from  the DFT components only when there are both the fixed as well as %DFT 
   	Given I am on RxClaim PlanAdministrator Menu
   	When I select Option "4" to navigate to "RxClaim Plan Maintenance"
   	And I select Option "1" to navigate to "Active Plan by Plan Code"
   	Then navigating to price schedule related screens with "<PlanCode>"
   	And I press "F12" Key
