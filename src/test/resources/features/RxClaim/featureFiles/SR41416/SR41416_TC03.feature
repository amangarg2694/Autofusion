...
As a RxClaim User I want to check claim is paid and POS rebate are deducted properly 
  ...
  Scenario Outline: To check if the POS rebate is adjusted from all the DFT components while calculating the patient pay when patient pay is based on multiple rules 
   	...
   	Then modifying the sequence for drug cost schedule with "<DrugSchedule>","<Code>"
   	And I press "F12" Key
