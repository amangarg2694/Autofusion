Feature: A manual claim is submitted with medicaid subrogation
 ...
 Scenario Outline: Check Patient Pay Table Negative Due is Y
    ...
    Then I go to  RxClaim PlanAdministrator Menu
 ...
 Scenario Outline: Check by Submitting Claim with Medicaid Subrogation to Y in RxClaim
   ...
   Scenario Outline: Check Claim note when Claim Submitted with Medicaid Subrogation to Y in RxClaim
    And I press "F14" Key
    And I verify the Claim Note "<ClaimNote>" exists in "ClaimNoteTable Details"
   
    And I verify the "<ClaimNote>"Prevented negative amount due in "<ClaimNote>"
    And I press "F12" Key
   ...
   Then Validate "Approved Due Amount" on "Price Information Page 1 of 3"
     
      Examples: 
      |PlanSearch|Table|BIN|Proc Ctr|Grp|Pharmacy|Rx Nbr|Rf|Member ID|Payee Override|Medicaid Subrogation|Prod: ID|Disp Qty|DS|PSC|Cost|Rx Origin|
      |$1.00|SR40760A|777777|KDPCN|KDGRP|apharm|875656744|00|mem842-01|kiranpayee|Y|00247064407|2|3|0|5|5|
